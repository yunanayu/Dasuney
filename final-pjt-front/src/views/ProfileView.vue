<template>
  <div>
    ...
    <button @click="follow">{{ isFollowing ? '언팔로우' : '팔로우' }}</button>
    ...
    <div class="category">
      <nav>
        ...
        <RouterLink to="/actorlike">배우b ({{ actorCount }})</RouterLink> |
        <RouterLink to="/directorlike">감독b ({{ directorCount }})</RouterLink> |
        <RouterLink to="/hopemovie">보고싶어요 ({{ hopeMovieCount }})</RouterLink>
      </nav>
    </div>
  </div>
  <RouterView />
...
// })
// const followingsCount = computed(() => {
//   return followings.length
// })
const ratingCount = ref(0);
const actorCount = ref(0);
const directorCount = ref(0);
const hopeMovieCount = ref(0);
...
// 여기서 각 페이지의 데이터를 가져와서 count 설정
  // 예시: 각 항목의 데이터를 가져와서 count 설정
  // ratingCount.value = 데이터.length;
  // actorCount.value = 데이터.length;
  // directorCount.value = 데이터.length;
  // hopeMovieCount.value = 데이터.length;
...
onMounted(() => {
  ...
  // console.log(store.tempUsername)
